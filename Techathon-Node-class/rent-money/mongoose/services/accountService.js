...
exports.updateAccountService=async(accountId,name,address,findAccount)=>{
    name !== undefined ? (findAccount.name = name) : findAccount;
    address !== undefined ? (findAccount.address = address) : findAccount;
    const updatedAccount = await AccountModel.findByIdAndUpdate(accountId,findAccount,{new:true});
